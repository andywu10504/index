...
function renderTimelineYears(startYear, endYear) {
  ...
  container.innerHTML = `
    <div class="timeline-years-label">年份</div>
    <div class="timeline-years-track">
      ${years.map(function (year) {
        return `<span>${year === endYear ? "Now" : year}</span>`;
      }).join("")}
    </div>
  `;
}
...
function getTimelineYearMarks(startYear, endYear) {
  ...
  return Array.from(new Set(importantYears)).filter(function (year) {
...
function renderTimelineRows(startYear, endYear, totalYears) {
  ...
  aboutProfile.timeline.forEach(function (item, index) {
    ...
    const detailId = "timelineDetail" + index;
    const isPoint = item.start === itemEnd;

    const row = document.createElement("div");
    row.className = "timeline-row";

    row.innerHTML = `
      <div class="timeline-label">${escapeHtml(item.title)}</div>

      <div class="timeline-track">
        <button type="button"
                class="${isPoint ? "timeline-dot" : "timeline-bar"}"
                style="left:${left}%; ${isPoint ? "" : "width:" + width + "%;"} background:${category.color};"
                data-bs-toggle="collapse"
                data-bs-target="#${detailId}"
                aria-expanded="true"
                aria-controls="${detailId}">
          ${isPoint ? "" : escapeHtml(formatPeriod(item.start, item.end, endYear))}
        </button>
      </div>

      <div class="collapse show timeline-detail" id="${detailId}">
        <div class="detail-card" style="border-left-color:${category.color};">
          <h3>${escapeHtml(formatPeriod(item.start, item.end, endYear))}｜${escapeHtml(item.title)}</h3>
          <p>${escapeHtml(item.desc)}</p>
          ${item.link ? `<a href="${item.link}" target="_blank" rel="noopener">查看相關連結</a>` : ""}
        </div>
      </div>
    `;
...
function renderLifeMosaic() {
  ...
  aboutProfile.lifeMosaic.forEach(function (item) {
    const card = document.createElement("div");
    card.className = "mosaic-card story-animate " + getMosaicSizeClass(item.ratio);
...
function getMosaicSizeClass(ratio) {
  if (ratio >= 30) {
    return "size-xl";
  }
